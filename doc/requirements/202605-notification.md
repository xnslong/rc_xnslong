...
**第三类：基于签名的鉴权** — 不是附加凭证，而是对请求内容本身做签名变换。这类已由 §3.6 覆盖。
...
- 合并原独立章节 §3（请求构建）和 §4（响应判定）为统一的 §3 领域能力分析，按"概念模型→数据契约→投递契约→映射→鉴权→签名→响应判定"组织
- 附录按使用频率重排序：A 调研详情（最常用参考）→ B 角色定义 → C Review 记录
