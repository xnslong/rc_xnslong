...
| **大部分供应商的请求构造需求落在简单映射范围**：从实际调研看，绝大多数供应商的 API 请求在字段映射层面属于直接取值或简单变换（即 §3.4 中契约复杂度低的语义），复杂的结构重组和异构格式是少数 | 若大部分供应商实际需要复杂的映射能力（结构重组或异构格式），则 MVP 对基础映射的投入可能不够，更多供应商需要扩展点或 SPI，削弱集中配置的优势 |
