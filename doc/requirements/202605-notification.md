...
- [3. 请求构造](#3-请求构造)
  - [3.1 问题的核心](#31-问题的核心)
  - [3.2 领域概念模型](#32-领域概念模型)
  - [3.3 核心思路：事件类型作为数据契约的锚点](#33-核心思路事件类型作为数据契约的锚点)
  - [3.4 数据契约的维护](#34-数据契约的维护)
  - [3.5 映射关系的几种组织方式](#35-映射关系的几种组织方式)
  - [3.6 映射规则需要表达的语义](#36-映射规则需要表达的语义)
...
外部供应商的 API 在请求格式（JSON / XML / SOAP）、鉴权方式（API Key / Bearer Token / HMAC 签名 / OAuth 2.0）、签名算法、成功判定规则上千差万别（详见第 4-6 节）。如果每个业务系统都直接对接供应商，那么每当新增一个供应商，所有需要通知它的业务系统都要重复实现这套接入逻辑；每当供应商升级 API，各业务系统又得同步修改。
...
- **异步 & 即抛即忘**：业务系统提交通知后立即返回，**不需要等待外部 API 响应**。但"忘"的是等待，不是状态——系统内部会持续追踪每条通知的最终送达状态，并提供可见性
- **多供应商适配**：支持不同外部系统的请求地址、Header、Body、鉴权方式差异
- **可靠送达**：通过持久化、重试、死信等机制保障通知最终送达
...
| **长期存储业务数据** | 通知送达后原始 payload 不长期保留（审计归档除外，见 §8.9）。系统不是业务数仓 |
...
| **映射规则** | Mapping Rule | 定义某个 (事件类型 × 供应商) 组合的转换逻辑——从 payload 的哪些字段取值，放到供应商请求的什么位置 |
...
利益相关者是所有对系统有利益关切的角色，包括直接与系统交互的参与者和其他间接相关方。

| 利益相关者 | 角色说明 | 核心关切与痛点 |
|-----------|---------|---------------|
| **业务系统（调用方）** | 提交通知的内部系统 | <ul><li>关切：提交通知后不被阻塞，能确认最终送达；新增或切换供应商时不修改自身代码</li><li>痛点：每个供应商都要重复做一遍集成，供应商的 API 变更直接牵连自己的代码，业务逻辑被外部接口细节淹没</li></ul> |
| **运维/运营人员** | 管理供应商接入、配置映射、处理死信 | <ul><li>关切：新增供应商的配置复杂度可控；死信能高效恢复</li><li>痛点：供应商接入分散在多个业务系统，没有统一管理入口；通知失败时排查链路长，日志碎片化</li></ul> |
| **外部供应商** | 接收通知的外部系统 | <ul><li>关切：收到符合其 API 规范的请求</li><li>痛点：来自不同业务系统的请求格式不统一、鉴权不一致，接入方质量参差不齐</li></ul> |
| **业务方管理者** | 负责业务效果达成的管理人员 | <ul><li>关切：通知成功率是否达标；供应商合作是否正常；能直观看到系统运行效果</li><li>痛点：通知发出后无法确认是否送达，供应商出问题时最后一刻才知道，缺乏量化的效果评估手段</li></ul> |
...
参与者（见 §1.5）是利益相关者中与系统有直接交互的子集。

## 1.5 参与者定义

以下定义用例中出现的参与者。参与者是系统外部的角色或系统，与系统发生交互。

| 参与者 | 说明 |
|--------|------|
| **业务系统（调用方）** | 在关键事件发生时提交通知的内部系统，如订单系统、CRM 系统。业务系统还是事件类型和事件数据的定义者——它们决定"发生了什么事件"以及"事件携带什么数据" |
| **运维/运营人员** | 负责通知系统日常运行的开发和运营人员，关注系统的配置管理、运行状态和异常处理 |
| **外部供应商** | 通知的接收方，是提供 HTTP(S) API 的外部系统或服务，如广告平台、CRM SaaS、库存系统等 |
...
| **行为** | 根据供应商配置的重试策略（如最大重试次数、重试间隔），在延迟后重新投递。达到最大重试次数后进入死信 |
...
| 项目 | 内容 |
|------|------|
| **场景** | 运营/开发人员配置或修改外部供应商的 API 信息和映射规则 |
| **解决的问题** | 新增或变更供应商时，无需修改代码或重启服务 |
| **行为** | 管理供应商及其 API 接入信息，同时管理事件类型与供应商 API 之间的数据映射规则。配置项见下方 |
...
**数据映射规则（详见第 3 节）：**
- 定义"哪个事件类型的哪些字段"映射到"供应商请求的哪个位置"
- 每条映射规则关联一个 (事件类型, 供应商) 组合
...
通知系统是业务系统和外部供应商之间的中介：

```
业务系统 ─── event + payload ───→ 通知系统 ───→ 供应商 API
```

这里存在**两个数据接口**：

1. **业务系统 → 通知系统**：业务系统提交的事件数据，格式由事件的类型约定
2. **通知系统 → 供应商 API**：发送给外部供应商的 HTTP 请求，格式由供应商定义

映射问题就是：**如何将接口 1 的数据，转换为接口 2 的请求**。这反过来也回答了另一个问题——业务系统应该按什么格式提交数据。
...
classDiagram
    ...
    供应商 "1" --> "1" 供应商配置 : 接入信息
    供应商 "1" --> "*" 投递配置 : 提供
    供应商配置 "*" --> "1" 投递配置 : 被共用（公共部分）
    事件类型 "1" --> "*" 投递配置 : 路由到
```

其中的几个关键关系需要特别说明：

- **通知 → 投递任务**：一条通知可拆分为多个投递任务（路由到多个投递配置），每个任务独立执行
- **事件类型 → 投递配置**：路由是多对多的——一个事件类型可路由到多个投递配置（投递到不同供应商或同一供应商的不同端点），一个投递配置只处理一个事件类型到一个端点的投递
- **投递配置**：完整描述怎样将一个事件类型的数据投递给一个供应商 API 端点——包括 API 端点（方法+路径）、映射规则（如何从 payload 构造请求）、重试策略、响应判定规则等。映射规则中引用的 payload 字段来自该事件类型定义的 payload 结构（数据契约）
- **供应商配置**：投递配置的公共部分——基础 URL、鉴权方式与凭证、签名方式等，这些信息不随具体事件类型和 API 端点变化。它不是独立存在的，而是投递配置中抽离出来的共享部分

### 3.3 核心思路：事件类型作为数据契约的锚点

一个自然的选择是：**以事件类型为纽带，串联两端**。

```
事件 "order.paid"
  ├── 数据契约 (Schema): { order_id, user_id, amount, currency, product_list }
  │
  ├── 路由: → 供应商 A (CRM)、供应商 B (库存系统)、供应商 C (广告平台)
  │
  ├── 映射 A (→ CRM):
...
- **事件类型定义数据契约**——`order.paid` 规定了 payload 的结构（有什么字段，各字段的类型和含义）
- **映射规则定义转换逻辑**——每个 (事件类型 × 供应商) 组合有对应的映射，从该事件的 payload 中提取并变换数据
- **两端通过事件类型关联**——业务按事件类型提交数据，映射按事件类型知道从什么结构提取
...
| **集成实施方** | 为每个 (事件, 供应商) 定义映射规则，使用事件契约中声明的字段进行映射 |
...
- 所有引用该事件类型的映射规则被**标记为可能需要更新**
- 新增供应商时，集成方为相关事件类型**补增映射规则**，不修改业务系统

### 3.5 映射关系的几种组织方式

从维护角度，映射关系可能有三种组织方式，各有侧重：

| 方式 | 说明 | 适用场景 |
|------|------|----------|
| **按供应商聚合** | 每个供应商一套映射规则，包含该供应商关心的事件类型的映射 | 供应商数量少且稳定，但每新增事件需修改多处 |
| **按事件类型聚合** | 每个事件类型一套映射规则，包含该事件要投递的所有供应商的映射 | 新增事件类型时集中管理，但新增供应商需修改多处 |
| **按 (事件, 供应商) 独立** | 每个 (事件, 供应商) 组合一条独立映射记录 | 完全解耦，新增事件或供应商互不影响，但配置数量增长快 |

三种方式的核心差异在于**映射记录的维度**——是按事件组织、按供应商组织、还是按笛卡尔积独立组织。这直接影响到管理界面的布局和配置的 CRUD 方式。
...
| 角色 | 核心职责 | 操作边界 |
|------|---------|---------|
| **业务方** | 定义事件类型的 Schema（数据契约）——事件有什么字段、类型、含义 | 不接触供应商配置 |
| **供应商维护者** | 管理供应商接入信息 + 事件→供应商的映射规则——怎么把事件数据转成供应商要的格式 | 只能引用 Schema 中已定义的字段，不修改 Schema 本身 |
| **系统维护者** | 维护系统基础设施、配置审批、兜底处理 | 不直接参与业务配置编辑 |

### B.2 映射规则的归属

映射规则是唯一的耦合点——它同时引用 Schema 字段和供应商配置。归属原则：**映射规则由供应商维护者管理**，受业务方定义的 Schema 约束（只能引用已有字段，不能自行发明），同时受供应商配置约束（只能路由到其负责的供应商）。

### B.3 隔离原则

| 原则 | 说明 |
|------|------|
| **横向隔离** | 业务方 A 不能改业务方 B 的 Schema；供应商 X 的维护者不能改供应商 Y 的配置 |
