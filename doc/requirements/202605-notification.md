...
> **业务系统怎么知道提交什么格式？** → 查**事件类型的数据契约**。每个事件类型有文档化的 payload schema，业务系统按这个 schema 提交。

> **映射逻辑怎么知道从什么格式提取数据？** → 映射规则**以事件类型 schema 为基础**。每个映射规则声明"我从 X 事件的 payload 的哪个字段取值"，这是明确的、可校验的。

> **格式变了怎么办？** → 事件类型 schema 的变更（字段增删、类型变更）应**通知到所有引用方**（映射规则），标记为待更新。
