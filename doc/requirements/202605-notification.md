...
- **供应商应自行处理幂等**：由于重复请求可能发生，供应商需要自行处理去重（如通过请求中的幂等键）。通知系统只保证至少送达一次
- **供应商业务响应无需系统介入**：供应商 API 返回的业务语义（如"用户不存在"），通知系统只需记录为失败原因，无需解析或回传。业务系统如果关心这些语义，可通过查询投递结果自行获知
- **部分投递失败无法通过自动重试恢复**：某些失败（如参数错误、鉴权过期、供应商 API 变更）即使多次重试也无法成功，需要人工介入判断和处理
- **通知系统不处理用户级凭证**：同一供应商的所有投递请求使用系统配置的平台级凭证。如果业务系统需要以终端用户身份调用供应商 API，由业务系统自行处理
- **payload 的数据主所有权在业务系统**：通知系统只临时记录 payload 用于投递，不承诺长期存储。业务系统应维护自己的数据副本
