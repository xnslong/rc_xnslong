...
以下假设和约束贯穿整个文档，是后续所有分析的共同前提。

#### 假设

- **供应商 API 基于 HTTP(S) 协议**：本系统处理的是外部系统通过 HTTP(S) 暴露的 API，不涉及进程内通信、消息队列或非 HTTP 协议的系统对接
...
- **幂等键由业务系统提供**：业务系统为每次提交提供一个唯一的幂等键，通知系统用它识别重复提交，不负责生成幂等键
- **供应商的接入配置及 API 响应格式相对稳定**：供应商变更 API 的频率远低于业务系统提交通知的频率。API 的升级或变更通过配置管理处理，不需要通知系统实时自动发现
- **供应商的 API 配额是已知的**：供应商对 API 的调用限制（每分钟 N 次、每天 M 次）是可以配置的，不要求系统自动探测配额上限
- **业务系统提交的数据已过业务校验**：通知系统只验证数据的格式（是否符合事件类型的数据契约），不验证数据的业务含义（如金额是否合理、库存是否足够）

#### 约束

- **投递保证为"至少一次"(At Least Once)**：系统保证每条通知至少被投递一次。对于因重复投递产生的多余请求，由业务系统提供的幂等键配合供应商去重来消解。系统不做"恰好一次"(Exactly Once) 的保证
- **不接入用户级凭证**：同一供应商的所有投递请求使用系统配置的平台级凭证（API Key、共享密钥、OAuth Client Credentials 等），不处理与具体事件关联的用户级凭证（如 OAuth Authorization Code 获取的终端用户 Token）。业务系统若需以终端用户身份调用供应商 API，应直接调用或将凭证作为事件数据传入
- **不处理供应商的业务响应**：供应商 API 返回的业务语义（如"用户不存在"、"订单已关闭"），系统只记录为失败原因，不解析也不代处理。这类错误的处理由业务系统从通知的投递结果中获知后自行处理
- **不长期存储 payload**：通知送达后，原始 payload 数据不长期保留（审计归档除外，见 §5.9）
- **投递失败最终收口于死信**：超过最大重试次数的投递进入死信状态，等待人工处理，不自动无限重试
- **供应商间故障隔离**：一个供应商的故障（响应慢、超时、不可用）不应影响其他供应商的正常投递。这是系统的核心非功能性保障（见 §5.2）
