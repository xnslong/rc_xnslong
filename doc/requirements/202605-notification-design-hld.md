...
flowchart LR
    ...
    WK[投递工作器] --> OBS[可观测性<br/>指标/日志/告警]
```

**关键设计点**：
- **配置源**：Git 仓库，目录按业务域和供应商分层组织，通过 PR 审批管控变更
...
- **角色治理**：通过 Git CODEOWNERS 将目录与需求分析 §3.2 中定义的三类角色对应，实现契约治理的隔离原则：
  ...
  /events/order/order.paid.yaml     @order-team        # 业务方：Schema 定义审批
...
- **审批不负责正确性，只负责授权**：正确性由测试保障，审批由 CODEOWNERS 授权。两者的职责互不重叠：
