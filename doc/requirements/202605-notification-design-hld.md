...
配置加载采用按业务方和供应商粒度的降级启动策略，错误不跨边界传播。

| 错误类型 | 影响范围 | 表现 |
|---------|---------|------|
| 路由文件加载失败 | 仅该 biz 的路由规则不可用 | 影响该 biz 所有事件类型的路由，其他 biz 正常 |
...
降级启动策略与 HLD §7.2 的运行时故障隔离原则一致——一个业务方或供应商的故障不应影响其他业务方或供应商的正常运行。
...
| 错误类型 | 责任人 | 受影响方 | 被动观察方 |
|---------|-------|---------|-----------|
| 路由文件加载失败 | biz 团队（该 biz 流量中断，修复 route.yaml） | vendor 维护者（该 vendor 流量归零） | — |
...
| **运行状态接口（第二阶段）** | ★★★★★ | 被动观察 | Admin API 提供配置健康度查询，dashboard 展示当前降级状态，也供告警系统定期轮询 |

- **指标打点**：各模块在关键路径上对运行结果打点计数（投递成功/失败数、配置加载失败数、受影响 biz/vendor 数量等），指标携带影响范围标签供告警路由。告警系统基于这些指标配置阈值规则，按角色路由通知。这是告警的主力，独立于日志系统。
- **结构化日志**：各类错误输出 ERROR 级别日志，包含错误来源、影响范围（biz / vendor / event_type）、错误原因、以及是否触发了降级启动。日志作为告警触发后的排查入口，而非告警的主要来源。
- **运行状态接口**（第二阶段）：Admin API 提供配置健康度查询，列出当前哪些组件处于降级运行状态及其原因。运维人员可通过 dashboard 或告警规则订阅此接口的变化。
