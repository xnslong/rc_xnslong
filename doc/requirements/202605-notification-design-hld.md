...
| **简明性（所见即所得）** | 配置结构即为输出结构——`$each` 块内的字段就是输出对象的字段，看到配置就能直接推断输出 | 配置是"路径指向"而非"结构描述"——读者需要将多个 `[]` 字段在心里组装成最终结构，输出形状需脑补不可直观感知 |
...
选择关键字块式方案的核心理由：**配置的第一原则是"所见即所得"**，歧义不可接受。路径通配符仅在纯字段投影场景有简洁优势，但不足以弥补歧义风险。若将来纯投影场景大量出现，可考虑将通配符作为关键字块式的简写语法糖使用。
...
- **当前元素引用名选 `elem`、`item` 还是 `this`？**：`elem` 是 element 的缩写，非配置领域常用词；`this` 在配置语境中容易和面向对象混淆；`item` 是自然语言——"数组中的每个 item"，在配置中读起来最直观。`@{item.product_id}` 语义自明：item 即当前元素，取其中的 product_id。
