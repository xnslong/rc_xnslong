...
- [0. 设计输入](#0-设计输入)
  - [0.1 系统定位与核心约束](#01-系统定位与核心约束)
  - [0.2 核心非功能性需求](#02-核心非功能性需求)
  - [0.3 易变点与稳定点](#03-易变点与稳定点)
  - [0.4 契约治理模型](#04-契约治理模型)
- [1. 设计指导思想](#1-设计指导思想)
  - [1.1 核心原则](#11-核心原则)
...
- [附录 A: 设计评审记录](#附录-a-设计评审记录)
  ...
  - [A.11 数据契约与投递契约管控方案评审](#a11-数据契约与投递契约的管控方案评审2026-06-01)
  - [A.12 数据转换方案评审](#a12-数据转换方案评审2026-06-01)
  - [A.13 `$format` 作用域评审](#a13-format-的作用域评审2026-06-03)
  - [A.14 "降级启动"术语准确性评审](#a14-降级启动术语准确性评审2026-06-04)
...
以下结论来自需求分析文档，是当前设计的直接依据。

### 0.1 系统定位与核心约束

| 项目 | 内容 |
|------|------|
| **系统定位** | 一个将业务系统与外部供应商 API 解耦的、可靠的异步通知管道 |
| **核心交互** | 业务系统异步提交通知，系统在后台完成投递，不阻塞业务主流程 |
| **投递承诺** | 至少一次投递，不丢失、可重试，最终失败进死信 |
| **领域模型** | 事件类型 → 数据契约（payload Schema），通知 → 投递任务 → 投递契约 |
| **不可协商** | 单向推送、系统不解析业务数据、不保证供应商可用性 |

### 0.2 核心非功能性需求

| 需求 | 说明 |
|------|------|
| **故障隔离** | 一个供应商的故障不影响其他供应商 |
| **自动化故障保护** | 持续失败时自动暂停投递，恢复后自动恢复 |
| **运行时可配置** | 供应商接入、路由规则、映射规则等变更不应要求重启服务 |
| **幂等处理** | 重复提交不产生重复投递，依赖业务系统提供幂等键 |
| **可观测性** | 每条通知的生命周期状态可查、可追踪 |
| **供应商 Quota 管理** | 配额配置、超额保护、超限处理 |
| **调用方管理** | 调用方身份标识、提交速率配额、调用方间隔离 |

### 0.3 易变点与稳定点

| 类别 | 内容 |
|------|------|
| **稳定（核心骨架）** | 交互模式（异步提交）、投递模式（持久化→HTTP）、核心状态机、通知→投递任务领域抽象 |
| **易变（配置驱动）** | 供应商 API 格式、鉴权方式、请求构造逻辑、重试策略、限流配置、事件类型、路由规则 |

### 0.4 契约治理模型

| 角色 | 职责 | 操作边界 |
|------|------|---------|
| **业务方** | 定义事件类型的 Schema（数据契约），声明路由授权 | 不涉及供应商接入信息 |
| **供应商维护者** | 管理供应商接入信息及投递契约（映射、重试、响应判定），以及控制条件 | 引用但不修改 Schema 字段 |
| **系统维护者** | 维护系统基础设施、配置审批兜底 | 不直接参与业务配置编辑 |
...
| **DeliveryTask（投递任务）** | 面向单个供应商的投递任务，代表"需要发给这个供应商"。包含状态、重试计数、下次重试时间。终态包括 SUCCEEDED、DEAD_LETTER（重试耗尽）和 IGNORED（控制条件判断不投递） | 每个 Notification 对应 N 条 |
...
- **角色治理**：通过 Git CODEOWNERS 将目录与需求分析 §3.2 中定义的三类角色对应，实现契约治理的隔离原则：
  ...
  ```
  # CODEOWNERS 示例
  /events/order/routes/order.paid.yaml         @order-team        # 业务方：路由授权审批
...
配置加载采用按业务方和供应商粒度的边界容错策略，错误不跨边界传播。

| 错误类型 | 影响范围 | 表现 |
|---------|---------|------|
| 路由文件加载失败 | 仅该事件类型的路由规则不可用 | 影响该事件类型的路由，其他事件正常 |
| Schema 文件加载失败 | 仅该事件类型的 Schema 不可用 | 该事件类型的入站 Schema 校验失效，其他事件不受影响 |
| 供应商基础配置加载失败 | 该 vendor 整体不可用 | 路由到此 vendor 的通知投递失败（0 个 task），其他 vendor 正常 |
| 投递契约文件加载失败 | 仅该 vendor+event 的投递契约不可用 | 该事件类型发往该 vendor 的投递任务失败（死信），不影响该 vendor 其他事件类型的投递 |
| 全局性错误（目录结构异常等） | 整个配置加载过程失败 | 系统拒绝启动 |

边界容错策略与 HLD §7.2 的运行时故障隔离原则一致——一个业务方或供应商的故障不应影响其他业务方或供应商的正常运行。
...
| 分类 | 含义 | 感知强度 | 告警方式 |
|------|------|---------|---------|
| **责任人** | 谁配置写错了，负责修复 | 强 | 即时告警（IM/电话） |
| **受影响方** | 被波及，需要感知事件发生，必要时协调责任人修复 | 中 | 主动通知（邮件/IM） |
| **被动观察方** | 受影响但无需主动关注 | 弱 | dashboard 可见，不主动推送 |

| 错误类型 | 责任人 | 受影响方 | 被动观察方 |
|---------|-------|---------|-----------|
| 路由文件加载失败 | biz 团队（该事件类型路由不可用，修复 route 文件） | vendor 维护者（该 vendor 对应事件的流量归零） | — |
| Schema 文件加载失败 | biz 团队（该事件流量中断，修复 schema 文件） | vendor 维护者（该 vendor 对应事件流量减少） | — |
| 供应商基础配置加载失败 | vendor 维护者（该 vendor 整体不可用，修复 vendor.yaml） | biz 团队（通知终态 FAILED，需协调 vendor 方修复） | — |
| 投递契约文件加载失败 | vendor 维护者（该 vendor 对该事件的投递失败，修复 contract 文件） | biz 团队（通知终态 FAILED，需协调 vendor 方修复） | — |
| 全局性错误 | 系统运维 | biz 团队 + vendor 维护者（系统不可用） | — |

感知能力（指标打点、结构化日志、运行状态接口）由 §7.5 可观测性统一提供。
...
> **`$format` 仅处理日期/时间格式转换**（unix_s、unix_ms、iso8601、date、自定义格式字符串）。重量、长度等其他单位的转换不在引擎范围内——它们应在业务系统与供应商之间直接约定统一单位，无需引擎层处理。时间格式之所以特殊，是因为业界标准繁多（秒级/毫秒级时间戳、ISO 8601、日期字符串、含时区/不含时区），且不同系统对时间精度的要求差异大，一个字段必须由引擎在源头和终端之间自动适配才能避免静默错误。
...
| **简明性（所见即所得）** | 配置结构即为输出结构——`$each` 块内的字段就是输出对象的字段，看到配置就能直接推断输出，配置错了也能一眼看出 | 配置是"路径指向"而非"结构描述"——读者需要将多个 `[]` 字段在心里组装成最终结构，无法简单看出配置是否藏有错误 |
...
> 以下对照需求分析 §5 的每项非功能性需求，说明当前设计方案中的对应落地措施。未覆盖的需求标注原因。

### 7.1 可靠性与送达保证

| 需求 | 设计响应 | 对应章节 |
|------|---------|---------|
| **至少一次投递** | 通知持久化写入 DB 后返回 ID；MQ 驱动投递流程；失败后自动重试至多 N 次 | §3.3 主数据流、§5.5.3 重试与死信管理 |
| **不丢失保证** | 同步路径止于 DB 写入，持久化后再返回；MQ 仅携带 ID，payload 由 DB 管理 | §3.3 主数据流、§4.1.1 MQ 与 DB 角色定位 |
| **最终送达保证** | 指数退避+抖动重试，最终失败进死信表，运维可手动重试或丢弃 | §5.5.3 重试与死信管理 |
| **幂等处理** | 效果幂等语义，`(caller_id, idempotent_key)` 唯一约束 + INSERT ON CONFLICT，7 天 TTL | §1.3 幂等语义 |

### 7.2 故障隔离与稳定

| 需求 | 设计响应 | 对应章节 |
|------|---------|---------|
| **供应商间隔离** | 按供应商分区队列，每个供应商独立 Worker 池，一个供应商的故障不阻塞其他供应商的投递 | §4.2 按供应商分区队列 |
| **故障保护** | 熔断器状态机（CLOSED → OPEN → HALF_OPEN），连续错误率超阈值自动暂停投递，探活成功恢复 | §5.5.2 限流器与熔断器 |
| **优雅关闭** | 设计原则中已提及，具体实现细节在后续详细设计中展开 | — |
| **自愈** | MQ 消息持久化 + Consumer 确认机制，宕机重启后未确认的消息重投；未完成的通知从 PENDING 状态恢复 | §4.1.1 MQ 与 DB 角色定位 |
| **消息堆积保护** | 按供应商分区队列天然隔离堆积范围，一个供应商的积压不影响其他供应商 | §4.2 按供应商分区队列 |

### 7.3 性能

| 需求 | 设计响应 | 对应章节 |
|------|---------|---------|
| **吞吐量** | 纯 PG 起点（单机 1-2 万 TPS），按需演进到 PG+Redis 热缓存；预分片（N=1024）为分库留扩展路径 | §4.1.3 存储层选型、§4.1.3 数据库扩展策略 |
| **投递延迟** | 同步路径极短——DB 写入后即返回；MQ 仅携带 50 字节 ID，payload 由消费者点查；秒级投递延迟随实例水平扩展稳定 | §3.3 主数据流、§4.1.1 MQ 仅携带 ID |
| **突发处理** | MQ 队列天然缓冲，突发流量时吸收峰值；Worker 按供应商独立消费，积压不丢消息 | §4.1.1 MQ 核心引擎定位 |

### 7.4 调用方管理与供应商 Quota 管理

| 需求 | 设计响应 | 对应章节 |
|------|---------|---------|
| **调用方身份标识** | 每个调用方有唯一身份标识，提交通知时必须携带 caller_id | §5.1 接收网关 |
| **调用方配额** | 按调用方设定提交速率上限 — 不在 MVP 范围内，后续迭代引入 | — |
| **调用方治理** | 单个调用方异常通过限流器隔离，不影响其他调用方 | §5.5.2 限流器 |
| **配额配置** | 供应商配额上限通过配置注入，不需系统自动探测 | §5.5.2 限流器 |
| **超额保护** | 令牌桶算法控制投递速率，接近配额上限时限流 | §5.5.2 限流器 |
| **配额监控** | 不在 MVP 范围内，后续支持 Prometheus 指标暴露 | — |
...
| 需求 | 设计响应 | 对应章节 |
|------|---------|---------|
| **日志追踪** | 每次投递尝试的结构化 JSON 日志（含 request/response），接入日志平台；全局 trace_id 串联通知生命周期 | §4.1.2 DeliveryAttempt 审计替代方案 |
| **投递指标** | 投递总数/成功/失败、队列深度、死信积压等关键指标统计打点 | §3.2 模块职责总览 |
| **通知状态查询** | 按时间/供应商/事件类型/状态筛选，查 DB 或 API 均可 | §3.2 模块职责总览 |
| **告警** | 基于指标打点和结构化日志，通过外部告警系统实现；系统本身不投递告警通知 | 见下方 |
| **数仓/BI** | 不在 MVP 范围内。后续通过 §7.9 数据导出机制将三路数据（Notification / DeliveryTask / DeliveryAttempt）同步至数仓，支撑审计和 BI 仪表盘 | §7.9 数据导出 |

#### 感知能力

感知遵循"系统产生信号，外围设施负责通知"的原则。

感知按两个通道组织：

| 通道 | 说明 | 谁受益 | 兜底能力 |
|------|------|--------|---------|
| **同步反馈** | API 响应码直接告知调用方 | 有调用入口的角色（如业务方调用接收网关） | ❌ 依赖调用方对响应码做了监控 |
| **异步可观测** | 指标打点 + 结构化日志 → 日志采集 → 按角色路由告警/dashboard | 所有角色 | ✅ 统一兜底，不依赖任何一方的监控配置 |

异步可观测是各角色感知错误的稳定渠道，同步反馈仅提供更及时的体验。各模块的感知需求（如配置加载错误的角色矩阵 §4.4）均通过此通道覆盖。

##### 感知数据来源

| 来源 | 可靠性 | 用途 | 说明 |
|------|--------|------|------|
| **指标打点（metrics）** | ★★★★★ | 告警主力 | 计数器（投递成功/失败、配置加载失败、队列深度等），告警系统直接采集，延迟低、不受日志系统堆积影响 |
| **结构化日志 (logs)** | ★★★☆☆ | 辅助排查 | ERROR 级日志提供详细的上下文（文件路径、错误原因、影响范围），用于告警触发后的归因和定位 |
| **运行状态接口（第二阶段）** | ★★★★★ | 被动观察 | Admin API 提供配置健康度查询，dashboard 展示当前部分可用状态，也供告警系统定期轮询 |

- **指标打点**：各模块在关键路径上对运行结果打点计数（投递成功/失败数、配置加载失败数、受影响 biz/vendor 数量等），指标携带影响范围标签供告警路由。告警系统基于这些指标配置阈值规则，按角色路由通知。这是告警的主力，独立于日志系统。
- **结构化日志**：各类错误输出 ERROR 级别日志，包含错误来源、影响范围（biz / vendor / event_type）、错误原因、以及配置加载是否发生了部分失败。日志作为告警触发后的排查入口，而非告警的主要来源。
- **运行状态接口**（第二阶段）：Admin API 提供配置健康度查询，列出当前哪些组件处于部分可用状态及其原因。运维人员可通过 dashboard 或告警规则订阅此接口的变化。
...
| 强度 | 告警方式 | 说明 |
|------|---------|------|
| **强** | 即时告警 | IM 或电话通知，分钟级响应 |
| **中** | 主动通知 | 邮件或 IM 群消息，小时级响应 |
| **弱** | 被动观察 | 通过 dashboard 或通知状态查询可见，不主动推送 |

**MVP 边界**：MVP 阶段仅实现结构化日志和基础指标打点。运行状态接口不在 MVP 范围内。外围告警系统通过指标和日志采集实现告警规则配置。
...
| 需求 | 设计响应 | 对应章节 |
|------|---------|---------|
| **新供应商接入成本** | 新增供应商只需配置仓库新增目录和文件，不需修改核心代码 | §4.4 配置内容、§5.3 请求拼装 |
| **配置变更** | Git + Webhook + 轮询，配置变更不需重启服务；运行时配置内存快照原子替换 | §4.4 配置发布 |
| **向后兼容** | 配置格式变更通过演进路径管理，MVP 阶段无需额外兼容设计 | — |

### 7.8 可测试性

| 需求 | 设计响应 | 对应章节 |
|------|---------|---------|
| **供应商模拟** | 提供 Mock Vendor Server，支持配置响应模板和故障注入 | 见下方详述 |
| **故障注入** | Mock Vendor 支持模拟超时、特定错误码、body 条件失败等场景 | 见下方详述 |

#### 7.8.1 端到端测试方案

**总体策略**：端到端测试使用真实的 PostgreSQL 和 RabbitMQ 实例，以编译产物的二进制文件启动一个真实的通知系统实例作为被测对象。测试脚本将通知系统视为黑盒，不直接访问其内部组件（DB、MQ），只通过与真实业务系统一致的接口与系统交互。测试脚本向通知系统提交通知后，通知系统处理后最终向 Mock Vendor 发出 HTTP 请求。测试脚本通过查询 Mock Vendor 的请求记录，验证实际发出的请求是否符合预期。
...
**测试 ID 串联与测试逻辑**：每轮测试中，测试脚本生成一个唯一测试 ID，以 payload 字段的形式随通知提交。通过投递契约的映射规则，该 ID 被拼接到发往供应商的 HTTP 请求 Header 中。Mock Vendor 通过 Header 中的 ID 识别这是哪一个用例的，记录收到的请求，并施加对应用例 mock 的行为。测试用例通过测试 ID 从 Mock Vendor 获取接收到的请求，验证请求行为。

**示例**——基本投递成功场景：

| 步骤 | 操作 |
|------|------|
| ① | 测试脚本生成唯一 test_case_id，并向 Mock Vendor 配置该 ID 对应的响应（如对 `PATCH /api/v3/contacts` 返回 200） |
| ② | 将 test_case_id 作为 payload 字段提交通知 |
| ③ | 通知系统同步返回 notification_id |
| ④ | 通知系统异步完成路由、映射、投递 |
| ⑤ | 测试脚本通过 test_case_id 查询 Mock Vendor 的请求记录 `/__requests/{test_case_id}` |
| ⑥ | 断言请求的 Header、Body 符合映射配置预期 |
...
sequenceDiagram
    ...
    participant SYS as 通知系统
    participant MV as Mock Vendor

    Note over TS,MV: ① 生成测试 ID
    TS->>TS: 生成唯一 test_case_id

    Note over TS,MV: ② 预配置 Mock Vendor 的响应
    TS->>MV: POST /__scenario/{test_case_id} (API 路径 + 响应模板)

    Note over TS,MV: ③ 提交通知
    TS->>SYS: POST /notifications (payload + test_case_id)
    SYS-->>TS: ④ 返回 notification_id

    Note over SYS: 异步投递
    SYS->>MV: HTTP 请求 (X-Test-Id: test_case_id)
    MV->>MV: 按预配置响应模板匹配并响应

    Note over TS,MV: ⑤ 查询请求记录
    TS->>MV: GET /__requests/{test_case_id}
    MV-->>TS: 请求记录列表

    Note over TS,MV: ⑥ 验证
    TS->>TS: 断言：Header、Body 等符合预期
```

**测试配置**：端到端测试使用与生产同构的配置，仅将供应商端点指向 Mock Vendor：

```yaml
# test/config/vendors/crm_system.yaml
base_url: "http://localhost:9800"
```

**MVP 阶段策略**：端到端测试作为开发流程的一部分，开发者本地运行。后续纳入 CI 流水线，每次 PR 自动触发。

#### 7.8.2 Mock Vendor Server

**定位**：一个轻量级 HTTP Server，作为测试环境中的"虚拟供应商 API"，替代真实供应商沙箱用于端到端测试。

**核心能力**：

| 能力 | 说明 | 用途 |
|------|------|------|
| **响应预配置** | 提供 `/__scenario/{test_case_id}` 端点，测试脚本可预先为指定测试 ID 配置 API 路径对应的响应模板 | 按测试用例独立控制 Mock Vendor 的行为 |
| **请求记录** | 记录收到的请求（URL、Header、Body），按测试 ID 提供查询接口 | 验证实际发出的请求是否符合预期 |
| **延迟注入** | 配置响应延迟时间（固定或分布） | 验证超时重试逻辑 |
| **错误注入** | 配置返回特定错误码（429、500、503、连接断开） | 验证限流、重试、熔断器行为 |

**测试 ID 识别**：Mock Vendor 从请求 Header 中提取测试 ID：

```yaml
# test/mock_vendor/config.yaml
test_id_header: "X-Test-Id"    # 从请求的此 Header 提取测试 ID
```

**响应预配置**：测试脚本通过 `/__scenario/{test_case_id}` 端点提前配置该测试用例的响应：

```json
// POST /__scenario/{test_case_id}  请求体 — 正常场景
{
  "endpoints": [
    {
      "method": "PATCH",
...
```json
// POST /__scenario/{test_case_id}  请求体 — 超时/异常场景
{
  "endpoints": [
    {
      "method": "PATCH",
...
**请求记录查询**：Mock Vendor 提供 `/__requests/{test_case_id}` 端点，返回该测试 ID 关联的所有请求记录：

```json
GET /__requests/test_case_001
[
  {
    "method": "PATCH",
    "path": "/api/v3/contacts",
    "headers": {
      ...
      "X-Test-Id": "test_case_001"
    },
    "body": "{\"properties\": {\"lifecyclestage\": \"customer\"}}",
    "received_at": "2026-05-25T10:30:05Z"
  }
]
```
...
test/
├── mock_vendor/
│   ├── config.yaml               # Mock Vendor 全局配置（含 test_id_header）
│   └── server.go                 # Mock Server 实现（独立二进制）
└── e2e/
    └── notification_test.go      # 端到端测试用例
```
...
### A.12 数据转换方案评审（2026-06-01）

**问题**：当源数据为数组、目标也需要以数组组织时，映射配置应采用何种表达方案。

**关注点**：

- **有哪些候选方案？**：存在两个大的方向。方向 A 是关键字块式——用 `$source` 指定源数组，再用一个 `$` 关键字包裹元素映射块，块内对当前元素字段做映射。方向 B 是路径通配符式——在输入输出路径中用 `[*]` 标记数组维度。方向 A 内部还存在关键词选型的分支：`$iterate_elem`、`$map`、`$each`、`$for`，以及当前元素引用名 `elem`、`item`、`this` 的选择。

- **关键字块式与路径通配符式如何抉择？**：路径通配符在纯字段投影（不重命名、不格式化、不加静态值）场景下非常简洁——一行一个路径即可。但它存在根本性歧义：同一层多个带 `[]` 的字段，输出是对象数组还是字段分组？例如 `items[].product_id` 和 `items[].quantity` 同时出现时，可以理解为 `[{product_id: ..., quantity: ...}]`（对象数组，按位置对齐）或 `{product_id: [...], quantity: [...]}`（字段分组，各自展开），语法上无法区分。这个歧义一旦误用，下游 API 请求体结构直接错误且难以排查。关键字块式的 `$source` + `$each` 组合则无歧义——块内所有映射明确作用于同一个数组元素，输出必然是对象数组，一眼可判。虽然比通配符多一层缩进，但歧义风险不可接受。

- **`$each` 与 `$source` 的配合方式是否一致？**：映射引擎已有一套 `$source + $format`（格式转换）和 `$source + $type`（类型转换）的语法体系。`$source` 始终表示"数据来源"，不负责"怎么处理"。数组遍历采用 `$source + $each` 保持了同样的结构——`$source` 指定源数组，`$each` 定义遍历规则。如果改用纯 `$each` 直接取值（取代 `$source`），则 `$each` 同时承担了"取值"和"遍历"两个职责，与单值场景中 `$format`/`$type` 不取值、必须配合 `$source` 的规则不一致。用户学到的"`$source` 是数据来源"规则在数组场景就不成立了，增加了认知负担。
...
**结论**：采用关键字块式方案，`$source` + `$each` 配合数组遍历，使用 `item` 作为当前元素引用名。路径通配符方案不采用，若将来纯投影场景大量出现，可考虑作为 `$each` 的简写语法糖使用。

---

### A.13 `$format` 的作用域评审（2026-06-03）

**问题**：`$format` 应该支持哪些类型的格式转换——仅日期/时间，还是扩展为通用的单位转换（重量、长度、金额等）？

**关注点**：
- **仅日期/时间是否足够覆盖主要场景？**：通知系统的核心是"事件通知"，不是通用数据集成平台。事件中最常见的格式不匹配就是时间戳格式——业务方可能传秒级时间戳（`1736870400`）、毫秒级时间戳（`1736870400000`）、ISO 8601 字符串、纯日期字符串（`"2026-05-24"`），而不同的供应商 API 又各自期望不同。一个字段一旦时间精度出错（差 1000 倍），下游数据全部偏移，这是静默数据错误中最危险的一类。金额精度、重量单位等虽然也是格式不一致，但后果是直观的数字偏差，排查成本低，而且更容易在业务系统和供应商之间的对接文档中提前约定统一。

- **扩展为单位转换会带来什么负担？**：单位转换不是简单的"数值 × 系数"。以重量为例，kg ↔ lb 是线性转换，但液体盎司和重量盎司是不同概念，汇率还有时效性。要做到可靠，引擎需要内置一个可配置的转换因子库，每个供应商对接都可能需要注册新的转换规则。这实际上是让引擎承担了它不该承担的职责——通知系统的边界是"投递"，不是"数据转换平台"。每增加一种转换类型，引擎的配置复杂度、测试覆盖面积、出错的可能性都成倍增长。

- **业务系统和供应商之间的单位能否直接约定？**：可以。接口对接阶段，双方约定以"元（分精度）"还是"元（元精度）"来传输金额，以"g"还是"kg"来传输重量，这些是在 API 设计阶段就能定的，不需要运行时转换。唯有时间戳格式例外——同一个系统内部都可能存在多种时间精度（秒级、毫秒级），且历史数据的格式无法通过"约定"统一。为少见的单位转换增加引擎能力，让所有场景都得为此付出复杂度，不值得。

**哲学**：引擎的能力边界应在"不引入能力就无法避免静默数据错误"的地方。业务系统和供应商之间的单位格式差异可以由对接约定解决，不需要引擎层支持。日期/时间格式的多样性是历史遗留问题，引擎必须处理。

**结论**：`$format` 仅处理日期/时间格式转换。其他格式差异应在业务系统与供应商的对接阶段约定统一，引擎不介入。

---

### A.14 "降级启动"术语准确性评审（2026-06-04）

**问题**：HLD §加载容错 和 DD §9.4 中使用的"降级启动"、"降级运行状态"等术语是否准确？

**关注点**：

- **"降级"语义与实际情况不匹配**："降级"意味着系统从较高的能力水平**降低**到较低的水平——如健康检查降级（从实时数据降为缓存）、熔断降级（从正常调用降为返回默认值）。但配置加载的场景不存在"从高处降下来"的过程——系统从未达到过全能力状态，而是在加载时就未能加载全部配置。这不是"降级"，而是"部分可用"。

- **系统重启后的行为说明不是"降级"**：假设运行中系统因某供应商配置加载错误处于部分可用状态，此时系统重启。如果重启后该供应商配置仍然报错而其他配置正常，系统重启前后的能力完全一致，没有"降"任何东西。如果重启后因同样的供应商配置导致系统启动失败，则运行时与重启时的行为不一致——一个普通滚动重启会让一个已报错的供应商配置拉垮整个集群，使运行时已有的容错变成"不可靠的陷阱"。边界容错的能力是配置加载层的固有属性，不是启动过程特有的行为。

- **"容错启动"也不准确**：容错是配置加载层的能力属性——任何时候加载配置（启动时、运行时热加载、Webhook 触发加载），行为都一样。如果把这一能力称为"容错启动"，就会暗示启动时额外做了容错，而实际上启动只是将系统恢复至与重启前一致的状态。

- **修正后的表达**：将"降级启动策略"修正为"边界容错策略"，将"降级运行状态"/"降级状态"修正为"部分可用状态"。核心原则不变——加载错误按业务方和供应商边界隔离，不跨边界传播，系统在部分配置加载失败时继续运行，服务于未受影响的配置。
