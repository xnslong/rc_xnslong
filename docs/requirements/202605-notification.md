...
| 轮次 | Review要点 | 修改概要 |
|------|-----------|----------|
| R4 | 文档缺乏系统定位、利益相关者、边界分析、领域模型等基础分析，结构不完整 | 新增五问分析(§1.0)、系统定位(§1.1)、利益相关者(§1.2)；区分约束与假设(§1.3)；补充系统边界(§1.4)、术语(§1.5)；新增用户故事(§2.0)；新增领域概念模型(§3.1)；合并 §3/§4 为统一领域能力分析；附录按调研→角色→Review 重排序 |
| R3 | §3.4 映射分层混入了方案判断；"数据结构重组"语义模糊 | 拆除 L1-L4 单维分级；替换为三维交叉矩阵（契约复杂度/需求旺盛度/需求复杂度）；"数据结构重组"替换为"数组遍历"；删除设计策略列 |
...
| R2 | 三类参与者角色定义——业务方/Schema、供应商维护者/映射、系统维护者/审批 | 新增附录 B 契约治理 |
...
- [3. 领域能力分析](#3-领域能力分析)
  ...
  - [3.2 契约治理](#32-契约治理)
    - [3.2.1 角色定义](#321-角色定义)
    - [3.2.2 投递契约的归属](#322-投递契约的归属)
    - [3.2.3 隔离原则](#323-隔离原则)
  - [3.3 数据契约的维护](#33-数据契约的维护)
  - [3.4 投递契约的组织方式](#34-投递契约的组织方式)
  - [3.5 映射规则需要表达的语义](#35-映射规则需要表达的语义)
  - [3.6 鉴权方式](#36-鉴权方式)
  - [3.7 签名需求](#37-签名需求)
  - [3.8 响应成功判定需求](#38-响应成功判定需求)
- [4. 变化点分析](#4-变化点分析)
...
- [5. 非功能性需求](#5-非功能性需求)
  ...
  - [5.9 数据导出](#59-数据导出)
- [6. MVP 范围](#6-mvp-范围)
- [附录 A — 供应商调研详情](#附录-a--供应商调研详情)
  - [A.1 请求格式与响应判定](#a1-请求格式与响应判定)
    - [A.1.1 请求格式的差异维度](#a11-请求格式的差异维度)
    - [A.1.2 供应商 API 示例](#a12-供应商-api-示例)
    - [A.1.3 响应状态码](#a13-响应状态码)
    - [A.1.4 "200 但实际失败"——各供应商案例](#a14-200-但实际失败各供应商案例)
  - [A.2 签名机制](#a2-签名机制)
    - [A.2.1 签名算法类型](#a21-签名算法类型)
    - [A.2.2 签名的通用构造模式](#a22-签名的通用构造模式)
    - [A.2.3 规范化 Header](#a23-规范化-header)
    - [A.2.4 Query 参数的规范化处理](#a24-query-参数的规范化处理)
    - [A.2.5 签名结果的携带方式](#a25-签名结果的携带方式)
    - [A.2.6 代理网关环境下的签名处理](#a26-代理网关环境下的签名处理)
  - [A.3 鉴权方式](#a3-鉴权方式)
    - [A.3.1 常见鉴权方式](#a31-常见鉴权方式)
- [附录 C — Review详情](#附录-c--review详情)
  - [C.1 "即抛即忘"的澄清](#c1-即抛即忘的澄清)
  - [C.2 Body template 不是需求](#c2-body-template-不是需求)
  - [C.3 去除技术方案细节](#c3-去除技术方案细节)
  - [C.4 供应商 Quota 超限问题](#c4-供应商-quota-超限问题)
  - [C.5 核心指标补充](#c5-核心指标补充)
  - [C.6 供应商鉴权方式调研](#c6-供应商鉴权方式调研)
  - [C.7 通知消息与 API 请求的数据映射](#c7-通知消息与-api-请求的数据映射)
  - [C.8 多调用方场景](#c8-多调用方场景)
  - [C.9 §3.4 映射语义分层重构](#c9-34-映射语义分层重构)
  - [C.10 系统框架与领域模型补充](#c10-系统框架与领域模型补充)
...
以下假设仅适用于 MVP 阶段，与 MVP 范围划定直接相关。随着系统演进和供应商/调用方数量增长，这些假设可能逐步不再成立。

| 假设 | 若不成立 |
|------|---------|
| **MVP 阶段对接的供应商数量有限（预计 2-3 个），且以 JSON 格式为主**：XML/SOAP 等异构格式的供应商暂不在首批对接范围内 | 需将异构格式支持提前到 MVP，映射引擎的复杂度将显著增加 |
| **MVP 阶段的调用方（业务系统）数量有限且均为内部系统**：调用方身份可由简单的静态 Token 或内网鉴权来管理，不需要复杂的调用方权限体系 | 需在 MVP 中加入调用方鉴权和管理功能（对应需求 #2） |
| **MVP 阶段供应商的接入由通知系统开发团队直接配置，不通过自服务管理后台**：配置变更通过配置文件或数据库直接操作即可 | 需在 MVP 中加入管理后台（对应需求 #21），增加前端开发工作量 |
| **MVP 阶段的通知量在可控范围内，单实例即可满足性能要求**：水平扩展和分库分表可在后续迭代中引入 | 需在 MVP 中加入水平扩展能力，架构复杂度显著增加 |
| **供应商在 MVP 阶段的鉴权方式以 API Key 和静态 Bearer Token 为主**：OAuth 2.0、HMAC 签名等复杂鉴权可在二期支持 | 若首个对接的供应商就要求 OAuth 2.0，MVP 范围需重新划定 |
...
| **长期存储业务数据** | 通知送达后原始 payload 不长期保留（审计归档除外，见 §5.5 数仓/BI 及 §5.9 数据导出）。系统不是业务数仓 |
...
为理解投递格式差异，以下是在实际调研中遇到的五种典型场景（详见[附录 A](#附录-a--供应商调研详情)），按请求构造复杂度递进排列：

- **固定模板**：请求结构固定，仅替换 payload 中的字段值。例如广告平台回传事件，所有 event 类型都用同一个请求骨架，只有 `user_id`、`timestamp` 这样的值不同，没有条件判断和结构变化
- **条件构造**：多个事件类型路由到同一个供应商，但该供应商的请求中包含哪些字段取决于具体的事件类型或数据值。**这是映射层面的条件逻辑，不是路由层面的差异**。例如 "order.paid" 和 "user.registered" 都投递给同一个 CRM 系统，但 order.paid 携带 `{lifecyclestage: "customer", last_paid_date: ...}`，user.registered 携带 `{lifecyclestage: "lead", source: "website"}`——同一供应商的请求结构随事件类型变化。或者同一事件中金额超阈值时多带一个 `approval_required: true` 字段
- **数据变换**：payload 的字段不能直接映射到供应商请求，需要重命名、嵌套、拆分或聚合。例如 payload 中有 `product_id`、`quantity`、`warehouse` 三个独立字段，但供应商要求 `{"items": [{"product_id": "...", "qty": 1, "location": "SH"}]}`——字段重命名（quantity→qty、warehouse→location）并包装到数组中
- **异构格式**：供应商要求非 JSON 格式，如 XML 或 SOAP 请求，整个请求体需要用不同的序列化方式构造
- **计算/签名**：发送时需要在请求中动态计算值（如 HMAC 签名、OAuth Token），这些值不能在配置时预计算，只能在实际发送时刻生成
...
先明确系统中涉及的几个核心业务实体及其关系，作为理解后续领域能力分析的基础：
...
| **时间戳格式转换** | 时间戳在不同精度或格式间转换 | 毫秒级时间戳转秒级 |
| **条件映射** | 根据某些条件决定是否包含某个字段，或取不同的值 | 若 `amount > 0` 放入 `body.status = "paid"`，否则放入 `body.status = "pending"` |
| **数组遍历** | 源数据中包含数组，目标中也以数组组织，数组内元素的字段映射规则与外层保持一致 | `payload.items[*].product_id → body.line_items[*].product_id`，源数组和目标数组按位置一一对应，每对元素应用相同的字段映射规则 |
| **异构格式** | 供应商要求非 JSON 格式（XML/SOAP），系统需要切换序列化方式构造请求体 | 金融/账单系统通知，要求 XML 或 SOAP Envelope 格式 |

这些语义的单次配置维护复杂度各不相同，需要从多个维度系统评估。以下从三个正交维度「**契约复杂度**（配置维护角度）、**需求旺盛度**（常见程度）和 **需求复杂度**（逻辑层次）」展开分析。

#### 3.4.1 映射语义 × 复杂度分析

| 映射语义 | 契约复杂度 | 需求旺盛度 | 需求复杂度 | 优先级 |
|---------|-----------|-----------|-----------|-------|
| 直接映射 | 低 | 高 | 低 | P0 |
| 字段重命名 | 低 | 高 | 低 | P0 |
| 静态值 | 低 | 中 | 低 | P1 |
...
- **契约复杂度**：低→可肉眼验证，看一眼即可判断对错；中→需上下文理解，依赖事件语义或格式约定；高→需工具验证，必须借助测试或预览
- **需求旺盛度**：高→几乎所有供应商接入都会遇到；中→部分供应商或事件类型有需求；低→偶发的供应商个例
- **需求复杂度**：低→单值单路径的复制或变换，本质上是一个映射原语；中→需额外逻辑层（条件分支、元素迭代），本质上是原语的组合使用；高→需不同的处理框架（不同序列化格式），涉及系统能力边界的跨越

> 需求复杂度与实现方案无关，衡量的是映射操作在逻辑上的复杂层次。优先级 P0 为契约复杂度低且需求旺盛度高的语义，映射引擎必须优先满足；P1 为基础能力完整覆盖；P2 使用频率中等且犯错成本较高，需辅以约束；P3 为偶发需求且跨越系统能力边界，可阶段性延后。

### 3.6 鉴权方式

每个供应商决定了其 API 使用什么鉴权方式，通知系统必须适配这些差异（详见[附录 A](#附录-a--供应商调研详情)）。从调研来看，不同鉴权方式对系统的要求可分为以下三类：
...
**第三类：基于签名的鉴权** — 不是附加凭证，而是对请求内容本身做签名变换。这类已由 §3.7 覆盖。
...
签名的作用是让供应商通过约定的算法验证请求方的身份和请求完整性。不同签名方案的核心结构相似——构造待签字符串 → 结合密钥 → 输出签名，但在**哪些部分参与签名、如何规范化等方面差异很大**（详见[附录 A](#附录-a--供应商调研详情)）。从调研来看，签名需求可归为两大类：
...
系统**不能仅靠 HTTP 状态码判断通知是否送达成功**，不同供应商的响应行为差异很大（详细案例见[附录 A](#附录-a--供应商调研详情)）。系统需要满足：
...
| 需求 | 说明 |
|------|------|
| **配额配置** | 支持为每个供应商配置 API 调用配额上限（如：每分钟 N 次、每天 M 次）。配额信息是运维的已知信息，通过配置注入，不需系统自动探测 |
...
| **数仓/BI** | 通知及投递数据落入数仓（Hive 等），支持长期审计和 BI 仪表盘搭建 | 后续 |
...
| 需求 | 说明 |
|------|------|
| **数据导出** | 通知处理完成后，原始通知及投递结果应能被下游系统消费或导出（如用于审计、BI 分析）。MVP 阶段不必实现，但需为导出预留扩展接口 |
...
**问题**：§3.4 原有的 L1-L4 四个层级以"实现复杂度"为单一维度对映射语义进行分级，且标注了"基础支持/扩展支持"等方案策略——混淆了需求分析与方案决策的边界。"数据结构重组"语义定义模糊，实际示例如嵌套路径映射和数组遍历均可被路径表达能力分解。

**结论**：
- 拆除 L1-L4 单维分级，替换为三维交叉分析（契约复杂度、需求旺盛度、需求复杂度）
- "数据结构重组"替换为"数组遍历"，契约复杂度由高降至低
- 时间戳格式转换的契约复杂度由中降至低，理由从"可统一约定"改为"显式声明足够直观"
- 删除原策略列（"内置基础能力/扩展支持/工具化支撑"），优先级别作为分析结论保留
- 与"需求文档不包含方案决策"的边界原则对齐

### C.10 系统框架与领域模型补充

**问题**：初始文档仅聚焦于投递管道的技术描述，缺乏从业务价值到系统边界到领域模型的完整分析链。具体缺失：五问分析（系统为什么存在）、利益相关者与痛点分析（为谁而做）、约束与假设的区分（哪些可协商）、系统边界（做什么/不做什么）、术语定义（沟通基础）、用户故事（用价值视角串联需求）、领域概念模型（核心实体的关系）。

**结论**：
- 新增 §1.0 五问分析，建立"业务痛点 → 技术后果 → 商业价值"的推导链，明确系统存在的理由
- 新增 §1.1 系统定位，提炼"解耦、异步、可靠"三个核心面向
- 新增 §1.2 利益相关者与参与者，每个角色附带核心关切与痛点
- 重构 §1.3，区分约束（不可协商的边界）与假设（可变化的前提，注明不成立的后果）
- 新增 §1.4 系统边界，用"✅ 负责 / ❌ 不负责"的对比明确职责范围
- 新增 §1.5 术语解释，统一文档中 12 个核心术语的定义
- 新增 §2.0 用户故事，从各参与者视角用故事方式描述系统价值
- 新增 §3.1 领域概念模型（含 mermaid classDiagram），明确调用方/供应商/事件类型/投递契约/通知/投递任务六要素及其关系
- 新增 §3.2 契约治理，涵盖角色操作边界、投递契约归属和隔离原则（原附录 B），将治理规则置于领域模型之后作为统一分析链的一部分
- 合并原独立章节 §3（请求构建）和 §4（响应判定）为统一的 §3 领域能力分析，按"概念模型→契约治理→数据契约→投递契约→映射→鉴权→签名→响应判定"组织
- 附录重排序：A 调研详情 → C Review 记录
