...
**端到端测试必须有对应的文档化测试用例定义。**

- 每个 `test/e2e/` 下的 `TestXxx` 函数，必须对应 `docs/notification-test-cases.md` 中的一个 `@test-case` 标注
- 测试用例必须从需求或边界推导出来，先有用例文档，再有测试代码
- 实现侧"顺便验证接口能通"的代码，放在对应模块的单元/集成测试中，不放在 `test/e2e/` 下
- 原因是：端到端测试验证的是系统的一组**明确定义的行为集合**，每个用例代表一个经过评审的、有文档的行为契约。随意添加测试代码而不记录用例，相当于往行为集合中塞入了一个未经评审的行为，污染了端到端测试的完整性。
